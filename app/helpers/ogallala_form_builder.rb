class OgallalaFormBuilder < ActionView::Helpers::FormBuilder
  # this is https://github.com/ncri/nested_form_fields, repurposed for my needs
  # (e.g. no remove subform button function)

  def nested_fields_for(record_name, record_object = nil, fields_options = {}, &block)
    fields_options, record_object = record_object, nil if record_object.is_a?(Hash) && record_object.extractable_options?
    fields_options[:builder] ||= options[:builder]
    fields_options[:parent_builder] ||= self
    fields_options[:wrapper_tag] ||= :li
  ...
  def add_nested_fields_link(association, text = nil, html_options = {}, &block)
    ...
    args << 'javascript:void(0)' # the actual href link
    args << {class: "#{html_class.empty? ? '' : html_class}",
  ...
  def add_default_classes_to_wrapper_options(association_name, wrapper_options)
    default_classes = ["subrecord-form-wrapper"]
